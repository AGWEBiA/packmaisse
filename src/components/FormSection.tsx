...
const FormSection = () => {
  ...
  return (
    <section id="form" className="py-24 sm:py-32 bg-background section-pattern scroll-mt-20 relative">
      ...
      <div className="max-w-lg mx-auto px-4 sm:px-6 relative z-10">
        <ScrollReveal>
          <div className="text-center mb-10">
            ...
            <h2 className="text-3xl sm:text-4xl font-bold text-foreground mb-3 font-[Poppins] tracking-tight">
        ...
        <ScrollReveal delay={0.2}>
          {submitted ? (
            <div className="glass-card rounded-2xl p-10 text-center glow-accent">
              <div className="w-20 h-20 rounded-full bg-accent/10 flex items-center justify-center mx-auto mb-6">
                <Check className="text-accent" size={36} />
              </div>
              <h3 className="font-[Poppins] font-bold text-foreground text-2xl mb-3">Obrigado!</h3>
              <p className="text-muted-foreground">Recebemos seus dados com sucesso. Em breve entraremos em contato!</p>
            </div>
          ) : (
            <div className="glass-card rounded-2xl p-7 sm:p-9 glow-primary">
              <Form {...form}>
                <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-5">
                  <FormField control={form.control} name="name" render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl>
                        <Input placeholder="Seu nome completo" className="h-12 rounded-xl bg-background/50" {...field} />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )} />
                  <FormField control={form.control} name="email" render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl>
                        <Input type="email" placeholder="seu@email.com" className="h-12 rounded-xl bg-background/50" {...field} />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )} />
                  <FormField control={form.control} name="whatsapp" render={({ field }) => (
                    <FormItem>
                      <FormLabel className="font-medium">WhatsApp com DDD</FormLabel>
                      <FormControl>
                        <Input type="tel" placeholder="(11) 99999-9999" className="h-12 rounded-xl bg-background/50" {...field} />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )} />
                  <Button
                    type="submit"
                    disabled={loading}
                    className="w-full bg-gradient-to-r from-accent to-emerald-500 hover:opacity-90 text-accent-foreground py-7 text-base font-bold rounded-xl shadow-lg shadow-accent/20 animate-pulse-soft"
                  >
